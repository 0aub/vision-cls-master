...
Writes log/bench-lesion-masks/:
    masks/<v8_filename>.png      binary ellipse mask, 256x256, raw orientation
    bench-lesion-bboxes.csv      one row per mask (v8 split, class, bbox, area)
...
def main():
    ...
    for n, r in enumerate(rows, 1):
        ...
        out_rows.append({
            ...
            "mask_frac": round(float(mask.mean()), 6),
    ...
    summary = {"masks_written": len(out_rows), "source_frames_missing": missing,
               "by_v8_split": dict(by_split), "by_class": dict(by_cls),
               "median_mask_px": float(np.median([r["mask_px"] for r in out_rows])),
               "median_mask_frac_pct": round(
                   100 * float(np.median([r["mask_frac"] for r in out_rows])), 3)}
