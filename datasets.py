        ...
        if printing:
            if self.cross_validation:
                ...
                for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels), 1):
                    ...
                    train_loader = DataLoader(image_datasets['train'], batch_size=batch_size, sampler=train_sampler, num_workers=2)
                    val_loader = DataLoader(image_datasets['train'], batch_size=batch_size, sampler=val_sampler, num_workers=2)
                    ...
                    for _, label in DataLoader(train_loader.dataset, batch_size=1, sampler=train_sampler):
                        train_class_counts[classes[label.item()]] += 1
